...
export async function getProductTypeBySlug(slug: string): Promise<ProductTypeRow | null> {
  const sql = getSql()
  const rows = await sql`
    SELECT id, name, slug, color, "hasHostingSpecs", active, "sortOrder", "createdAt", "updatedAt"
...
export async function createProductType(data: ProductTypeInput): Promise<ProductTypeRow> {
  ...
  const rows = await sql`
    INSERT INTO "ProductType" (
      ...
      ${data.sortOrder}, ${now}, ${now}
    )
    RETURNING id, name, slug, color, "hasHostingSpecs", active, "sortOrder", "createdAt", "updatedAt"
  `
